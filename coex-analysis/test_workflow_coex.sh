...
#   - List of HPO terms we wish to analyse. We create it in this script with --> ~/TFM/annotations/tis_hpo_ngenes_20220711.tsv
...
if [ "$HPO_LIST" = "all" ]; then
    HPO_FILE=~/TFM/annotations/tis_hpo_ngenes_20220711.tsv
...
for HPO in ${HPO_NAMES[@]}; do
    ...
    if [ ${#FINDPLOTS[@]} -eq 0 ]; then
        Rscript ~/TFM/coex-analysis/plot_coex.R -t $TIS -c $((NCLUS-1)) -o $HPO -r 10
    fi
done
