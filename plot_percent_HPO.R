...
data <- data[intersect != 0,]


data1 <- data[tissue %in% tejidos[1:8],]
data_bron <- data[tissue == "Bronchus",]
data_col <- data[tissue == "Colon",]
...
data_skmus <- data[tissue == "Skeletal muscle organ",]
data_sto <- data[tissue == "Stomach",]
...
plot1 <- ggplot(data1, aes(x = tissue, y = percent)) + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75), fill = "#99ffcc") +
         ...
         geom_boxplot(data = data_bron, width = 0.1, fill = "white", outlier.shape = NA) +
         geom_boxplot(data = data_col, width = 0.1, fill = "white", outlier.shape = NA) +
         labs(x = NULL, y = NULL) +
         theme_bw()
...
plot3 <- ggplot(data3, aes(x = tissue, y = percent)) + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75), fill = "#99ffcc") +
  ...
  geom_boxplot(data = data_skmus, width = 0.1, fill = "white", outlier.shape = NA) +
  geom_boxplot(data = data_sto, width = 0.1, fill = "white", outlier.shape = NA) +
...
pdf('plots/percent_HPO_nozeros.pdf')

plot

invisible(dev.off())

message("Plot saved! -> plots/percent_HPO_nozeros.pdf")
